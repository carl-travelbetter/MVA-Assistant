console.log("Week B Develop Tracker...");

let slTimetable = [];
 fetch('week-b-1-lesson-timetable.json')
...
const WEEK_B_DEVELOP_STORAGE_KEY = "mva_b_sldevelop_tracker";
let state = JSON.parse(localStorage.getItem(WEEK_B_DEVELOP_STORAGE_KEY)) || {position: []};

const B_SLDEVELOP_PRIORITY_KEY = "b_sldevelop_priority";
let savedPriority = localStorage.getItem(B_SLDEVELOP_PRIORITY_KEY) || {value: "1"};
let priority = 0;
...
function saveState() {
  ...
  localStorage.setItem(WEEK_B_DEVELOP_STORAGE_KEY, JSON.stringify(state));
}
...
function increasePriority() 
{
  ...
  if (priority > 10)
  {
    alert("All Develop Tasks Done - Well Done Georgie");
  ...
  localStorage.setItem(B_SLDEVELOP_PRIORITY_KEY, JSON.stringify(priority));
}
   ...
   slTimetable.forEach (item => 
    {
      if (item.priority === priority)
      {
        ...
        dueDay.textContent = "Due by "+lookupDay(14);
