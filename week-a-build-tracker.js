...
const WEEK_A_BUILD_STORAGE_KEY = "mva_a_slbuild_tracker";
let state = JSON.parse(localStorage.getItem(WEEK_A_BUILD_STORAGE_KEY)) || {position: []};

const A_SLBUILD_PRIORITY_KEY = "a_slbuild_priority";
let savedPriority = localStorage.getItem(A_SLBUILD_PRIORITY_KEY) || {value: "1"};
let priority = 0;
...
function saveState() {
  ...
  localStorage.setItem(WEEK_A_BUILD_STORAGE_KEY, JSON.stringify(state));
}
...
function increasePriority() 
{
  ...
  localStorage.setItem(A_SLBUILD_PRIORITY_KEY, JSON.stringify(priority));
}
   ...
   slTimetable.forEach (item => 
    {
      if (item.priority === priority)
      {
        ...
        nextDay = item.day+1;
        let dayLabel = lookupDay(nextDay);
...
function updateDoneList()
{
 ...
 state.position.forEach(item => {
  ...
  if (item.status == "Done")
  {
     ...
     done_task.textContent = "Subject "+item.subject+" Build "+" Status "+item.status;
