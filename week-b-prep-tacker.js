console.log("Week B Prep Tracker...");
...
const WKB_PREP_STORAGE_KEY = "mva_b_prep_sl_tracker";
let state = JSON.parse(localStorage.getItem(WKB_PREP_STORAGE_KEY)) || {position: []};

const B_SLPREP_PRIORITY_KEY = "b_slprep_priority";
let savedPriority = localStorage.getItem(B_SLPREP_PRIORITY_KEY) || {value: "1"};
let priority = 0;
...
function saveState() {
  ...
  localStorage.setItem(WKB_PREP_STORAGE_KEY, JSON.stringify(state));
}
...
function increasePriority() 
{
  ...
  localStorage.setItem(B_SLPREP_PRIORITY_KEY, JSON.stringify(priority));
}
